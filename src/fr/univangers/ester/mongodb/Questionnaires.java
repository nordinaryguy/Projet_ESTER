...
public class Questionnaires extends Database {
	...
	public void addQuestionnaire(String name, String identifiant, String html, String identifiantEster) {
		MongoCollection<Document> questionnaires = database.getCollection(C_QUESTIONNAIRE);
	...
	public String getHTMLQuestionnaire(String identifiant) {
		if(!existQuestionnaire(identifiant)) {
			throw new IllegalArgumentException("Le questionnaire n'existe pas.");	
		}
		MongoCollection<Document> questionnaires = database.getCollection(C_QUESTIONNAIRE);
	...
	public List<String> getIdentifiantQuestionnaires() {
	    ...
	    while(cursor.hasNext()) {
		    identifiants.add(cursor.next().get(IDENTIFIANT).toString());
	    }
	    return identifiants;
	}
